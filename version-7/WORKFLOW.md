...
│  │    ├─ "Confidence: 92%"                              │ │
│  │    ├─ "happy: 45 | neutral: 23 | ..."              │ │
│  │    └─ Emotion Trend Graph (auto-updated)            │ │
│  └────────────┬───────────────────────────────────────────┘ │
│               │                                              │
...
### Emotion History & Graph Generation
...
│  │    setEmotionHistory([...array])                      │ │
│  └────────────┬─────────────────────────────────────────┘ │
│               │                                              │
│               ▼                                              │
│  ┌────────────────────────────────────────────────────────┐ │
│  │ 3. Call updateChart(history)                          │ │
│  │    ├─ Prepare Chart.js data structure                 │ │
│  │    ├─ 7 datasets (1 per emotion type)                │ │
│  │    ├─ Color-code each emotion                        │ │
│  │    ├─ Plot points on line chart                      │ │
│  │    └─ Update or create Chart instance                │ │
│  └────────────┬─────────────────────────────────────────┘ │
│               │                                              │
│               ▼                                              │
│  ┌────────────────────────────────────────────────────────┐ │
│  │ 4. Chart Auto-Updates in Real-Time                    │ │
│  │    ├─ Shows last 50 emotion detections                │ │
│  │    ├─ X-axis: Detection count (1-50)                 │ │
│  │    ├─ Y-axis: Emotion present/absent (binary)        │ │
│  │    ├─ 7 colored lines (one per emotion)              │ │
│  │    └─ Legend at bottom                                │ │
│  └────────────────────────────────────────────────────────┘ │
│                                                              │
│  Clear Button:                                              │
│  └─ Reset all history → graph disappears                   │
│  └─ Reset stats → counts go to 0                           │
...
└─ Stats Section
   ...
   │
   └─ EmotionGraphDisplay (Conditional)
      └─ Chart.js line chart
         ├─ Updated each detection cycle
         ├─ Shows all 7 emotions
         ├─ Color-coded lines
         ├─ Auto-appears after first detection
         └─ Destroys when "Clear" button clicked
```
...
│  └─ [ { emotion: 'happy', confidence: 0.92, time: 123456 }, ... ]
│
└─ (refs)
   ...
   ├─ chartRef → <canvas> for Chart.js graph
   ├─ chartInstanceRef → Chart.js instance object
   ├─ emotionHistoryRef → Internal history (max 50)
...
```
Emotion Chart (Chart.js Line Graph)
├─ Type: Line chart
├─ Data Source: emotionHistory (last 50 detections)
├─ X-Axis: Detection count (1, 2, 3, ... 50)
├─ Y-Axis: Binary (0 or 1 for each emotion)
├─ Datasets: 7 lines (one per emotion)
│
└─ Features:
   ├─ Animation: Disabled (updates every 100ms)
   ├─ Responsive: true (scales to container)
   ├─ No tooltips (clean display)
   ├─ Legend: Bottom, small font
   └─ Colors: Matches emotion display colors
      ├─ Happy: Green (#2ee59d)
      ├─ Sad: Red (#ff5c7a)
      ├─ Angry: Red (#ff5c7a)
      ├─ Disgusted: Red (#ff5c7a)
      ├─ Fearful: Red (#ff5c7a)
      ├─ Surprised: Yellow (#ffd166)
      └─ Neutral: Gray (#a0a7b4)

Graph Lifecycle:
├─ Creation: First detection when emotionHistory.length > 0
├─ Update: Each detection calls updateChart(history)
│  └─ Data labels updated (1-50 range)
│  └─ Dataset values recalculated
│  └─ Chart.update() called (non-animated)
├─ Destroy: When "Clear" button clicked
│  └─ chartInstanceRef.current.destroy()
│  └─ Graph HTML removed
│  └─ New chart will be recreated on next detection
└─ Lifecycle Complete
```
...
```
Chart.js Instance:
├─ Created once per emotion history
├─ Stored in chartInstanceRef.current
├─ Updated 10x per second (non-destructively)
├─ Destroyed on "Clear" button → creates new on next detection
└─ Max memory: ~20-50MB for 50 detection points

Emotion History Array:
├─ Max size: 50 items (oldest removed when adding 51st)
├─ Memory per item: ~100 bytes
└─ Max total: ~5KB (negligible impact)
```
...
├─ Stacked button layout (wrap: true)
├─ Single column grid (stats)
├─ Font: 80% of desktop
└─ Touch targets: min 42px height

Tablet Layout (480px - 768px)
├─ Flexible widths
├─ Adjusted spacing
├─ 2-column grid (stats, if room)
├─ Font: 85% of desktop
└─ Touch targets: 44px height

Desktop Layout (> 768px)
├─ Fixed 720px width
├─ Flex controls (horizontal)
├─ Full grid display
├─ Regular font sizes
└─ Optimal spacing
```
...
```
Video Container:
├─ Desktop: width: 720px, fixed
├─ Tablet: width: min(720px, 100%)
├─ Mobile: width: 100%
└─ Aspect ratio: Always maintained

Controls:
├─ Desktop: flex row, gap: 10px
├─ Tablet: flex row, gap: 8px
├─ Mobile: flex wrap, flex: 1 per button
└─ All: min-height 42px

Statistics Grid:
├─ Desktop: flex-wrap: wrap, gap: 14px
├─ Tablet: flex-wrap: wrap, gap: 10px
├─ Mobile: flex-wrap: wrap, gap: 8px
└─ Adaptive: Fewer items visible on smaller screens
